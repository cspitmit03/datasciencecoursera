...
From the analysis to follow, it became clear tha although there are quantifiable differences in one transmission type being better than the other, other more appropriable variabels were better estimates of fuel efficiency. In addition, the data set would benefit from having more data points (n>32) in order to have higher degrees of confidence in the results.
...
In looking at the matrix, it appears as though MPG can be explained by the following variables cyl, hp and wt without really taking into consideration am.  A visual of this is shown in Appendix A.  Let's explore this more with actual regression models.
...
Next we will use the step function to determine what variables do have an influence on MPG.  With the use of AIC as a model quality estimator we note that the model lm(mpg ~ cyl + am + hp + wt) is the most effective. For a full detail look, please review Appendix B.
...
The point of concern on this model is that the variables cyl and am are used, but only for portions of factors (cyl 6/8 and Manual respectively). In my opinion, this is not a complete model although R-squared is 0.8401.

I have further whitled down my model and finalized it at mpg as a variable of weight and horsepower. This has a value of 0.815. Which is just as strong as the model with cylinder as a part of the equation.
...
In reviewing this plot, we see that it is not entirely random/homoscedastic, there seems to be some heteroscedastic patterns in the form of a parabola where the ends have higher positive variance and the middle has high negative variance

###Conclusion
Various analyses were done to determine the correlation of MPG across transmission type and other variables.  It was determined through various regression models that Horsepower and Weight were better variables used to predict MPG instead of the transmission type. 
